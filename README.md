...
-----------------------------------------------------------------------------------------------------------------------------
10.10.123.1               e8:39:35:ed:d6:30 Hewlett Packard           Hp                       eth4       DHCP       STALE
10.10.123.4               28:70:4e:32:36:5e Ubiquiti Inc              USW-Flex-Mini            eth4       DHCP       DELAY
10.10.2.100               00:13:1c:21:ca:84 LiteTouch, Inc.           *                        eth1       DHCP       STALE
10.10.2.4                 b0:6e:bf:bb:5c:61 ASUSTek COMPUTER INC.     PC                       eth1       DHCP       REACHABLE
169.254.216.15            b0:6e:bf:bb:5c:61 ASUSTek COMPUTER INC.     PC                       eth1       DHCP       STALE
10.10.5.123               a6:0d:e5:cc:21:aa -                         iPad                     phy0-ap0   DHCP       STALE
192.168.0.1               9c:53:22:de:10:43 TP-Link Systems Inc       -                        eth0       Static/?   REACHABLE
192.168.0.10              00:13:1b:4a:a0:a9 -                         -                        eth0       Static/?   STALE
192.168.0.238             -                 -                         -                        eth0       Static/?   FAILED
```
...
## Install
You need to have basic commands (grep, awk, sed etc...) available.
```
cat <<'THE_END' > /bin/list-clients && chmod +x /bin/list-clients && sed -i -e 's/\t//g' -e '/^[[:space:]]*$/d' /bin/list-clients && grep -qxF '/bin/list-clients' /etc/sysupgrade.conf || echo '/bin/list-clients' >> /etc/sysupgrade.conf
#!/bin/sh
...
Now you can simply run it as `list-clients` and add it as Custom command to LuCi!